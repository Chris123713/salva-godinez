...
function GiveCardItem(playerId, cardId, cardNumber, expirationDate, cvv, displayName, accountType)
    ...
    local expExists, canAdd = pcall(function() return inventory:CanCarryItem(playerId, cardItem, 1) end)

    if (expExists and canAdd) or not expExists then
        local success = inventory:AddItem(playerId, cardItem, 1, nil, {
            cardId = cardId,
...
for k, _ in pairs(Config.CardItems) do
    inventory:CreateUsableItem(Config.CardItems[k], function(source, item)
        if item then
