...
-- Store the dispatch route blip so we can remove it when responding to a new call
local dispatchRouteBlip = nil

---@param dispatch DispatchNotification
function RespondToDispatch(dispatch)
    local x, y = dispatch.location.coords.x, dispatch.location.coords.y

    -- Remove existing dispatch route blip if one exists
    if dispatchRouteBlip and DoesBlipExist(dispatchRouteBlip) then
        RemoveBlip(dispatchRouteBlip)
        dispatchRouteBlip = nil
    end

    -- Create a blip at the dispatch location for the purple GPS route
    dispatchRouteBlip = AddBlipForCoord(x, y, 0.0)
    SetBlipRoute(dispatchRouteBlip, true)
    SetBlipRouteColour(dispatchRouteBlip, 27) -- 27 = Purple color

    -- Also set the standard waypoint marker on the map
    SetNewWaypoint(x, y)
