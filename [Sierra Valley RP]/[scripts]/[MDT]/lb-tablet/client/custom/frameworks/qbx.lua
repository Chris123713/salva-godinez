...
function GetCompanyData()
    ...
    jobData.balance = AwaitCallback("qbx:services:getAccount") or 0
    jobData.employees = AwaitCallback("qbx:services:getEmployees") or {}
    jobData.grades = {}

    -- DEBUG: Log what we got from callbacks
    print("^3[LB-Tablet Debug] Balance: " .. tostring(jobData.balance))
    print("^3[LB-Tablet Debug] Employees count: " .. tostring(#jobData.employees))
    print("^3[LB-Tablet Debug] Employees data: " .. json.encode(jobData.employees))

    local sharedJob = QB.Shared.Jobs[jobData.job]
    if sharedJob and sharedJob.grades then
        for k, v in pairs(sharedJob.grades) do
    ...
    -- DEBUG: Log full company data before sending to UI
    print("^3[LB-Tablet Debug] Grades count: " .. tostring(#jobData.grades))
    print("^3[LB-Tablet Debug] Full jobData: " .. json.encode(jobData))
