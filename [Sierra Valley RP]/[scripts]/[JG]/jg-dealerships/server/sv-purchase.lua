...
local function purchaseVehicle(src, dealershipId, coords, purchaseType, society, societyType, model, colour, paymentMethod, financed, directSaleUuid)
  ...
  if purchaseType == "society" and paymentMethod == "societyFund" then
    ...
    -- Notify pd_boss_menu of society vehicle purchase for transaction logging
    local playerInfo = Framework.Server.GetPlayerInfo(src)
    local playerName = playerInfo and playerInfo.name or "Unknown"
    TriggerEvent('pd_boss:server:societyVehiclePurchase', {
      department = society,
      amount = amountToPay,
      vehicle = model,
      plate = plate,
