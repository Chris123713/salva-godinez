...
function AddToSociety(job, amount)
    ...
    elseif IsResourceStarted('bcs_companymanager') then
        exports.bcs_companymanager:AddCompanyMoney(job, 'money', amount)
    end
end
    ...
    if xTarget and source then
        MySQL.Async.execute("DELETE FROM licenses WHERE owner = @owner and license = @license", {
            ...
            if result then
                ...
                TriggerClientEvent('LicenseManager:notify', source, locale('license_manager'),
                    string.format(locale('license_revoked'), Config.Licenses[license].label), 'error', 3000)
            end
        end)
        ...
        for k, v in pairs(Config.Licenses) do
            RegisterUsableItem(k, function(source)
                ...
                elseif Config.inventory == 'qb-inventory' then
                    local item = exports['qb-inventory']:GetItemByName(source, k)
                    TriggerClientEvent('LicenseManager:client:UseCard', source, k, item.info)
                elseif Config.inventory == 'tgiann' then
                    local itemData = exports["tgiann-inventory"]:GetItemByName(source, k)
                    TriggerClientEvent('LicenseManager:client:UseCard', source, k, itemData.info)
                elseif Config.inventory == 'codem-inventory' then
                    local itemData = exports["codem-inventory"]:GetItemByName(source, k)
                    TriggerClientEvent('LicenseManager:client:UseCard', source, k, itemData.info or itemData.metadata)
                else
                    TriggerClientEvent('LicenseManager:client:UseCard', source, k)
                end
            end)
        end
        for department, _ in pairs(Config.Badges) do
            RegisterUsableItem(Config.BadgeItemName(department), function(source)
                local xPlayer = GetPlayerFromId(source)
                if Config.inventory == 'qs-inventory' then
                    local inventory = exports['qs-inventory']:GetInventory(xPlayer.source)
                    local item = GetItem(inventory, Config.BadgeItemName(department))
                    local metadata = item.info
                    TriggerClientEvent('LicenseManager:client:UseBadge', source, department, metadata)
                elseif Config.inventory == 'core_inventory' then
                    local inventory = exports['core_inventory']:getInventory('content-' ..
                        xPlayer.identifier:gsub(":", ""))
                    for j = 1, #inventory do
                        if inventory[j].name == Config.BadgeItemName(department) then
                            TriggerClientEvent('LicenseManager:client:UseBadge', source, department,
...
function IsLicenseValid(license)
    if not Config.Licenses[license] then
        print(('LicenseManager: License %s does not exist, please check your config/licenses.lua'):format(license))
        return false
    elseif not Config.Licenses[license].id_length then
        print(('LicenseManager: License %s does not have an id_length, please check your config/licenses.lua'):format(
            license))
        return false
    end
...
lib.callback.register('LicenseManager:server:RequestScreenshot', function(source)
    ...
    local response = exports.bcs_licensemanager:TakeScreenShot(xPlayer.source, upload)

    return response.success and response.data or false
end)

function UpdateMugshot(source, license, mugshot)
    local xPlayer = GetPlayerFromId(source)

    if not xPlayer then return end

    local data = MySQL.scalar.await("SELECT data FROM licenses WHERE owner = ? AND license = ?", {
        xPlayer.identifier,
        license
    })

    if not data then
        print(('LicenseManager: Failed to update mugshot, data not found in database owner : %s, license : %s'):format(
    ...
    local response = MySQL.update.await("UPDATE licenses SET data = ? WHERE owner = ? AND license = ?", {
        json.encode(data),
        xPlayer.identifier,
        license
    })
...
exports('UpdateMugshot', UpdateMugshot)

function GenerateLicenseMetaData(source, licenses)
    local xPlayer = GetPlayerFromId(source)
    if type(licenses) == 'string' then
        if not IsLicenseValid(licenses) then return nil end
        return GenerateLicenseData(licenses, xPlayer)
    elseif type(licenses) == 'table' and table.type(licenses) == 'array' then
        local data = {}
        for i = 1, #licenses, 1 do
            if not IsLicenseValid(licenses[i]) then return nil end
            data[licenses[i]] = GenerateLicenseData(licenses[i], xPlayer)
        end
        return data
    else
        print(('LicenseManager: type of licenses is not valid'))
        return nil
    end
end

exports('GenerateLicenseMetaData', GenerateLicenseMetaData)
