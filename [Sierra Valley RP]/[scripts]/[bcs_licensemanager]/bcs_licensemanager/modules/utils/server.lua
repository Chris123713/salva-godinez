function InventoryAddItem(player, item, count, data)
    ...
    elseif Config.inventory == 'tgiann' then
        exports["tgiann-inventory"]:AddItem(player.source, item, count, nil, data)
    elseif Config.inventory == 'qb-inventory' and IsResourceStarted('qb-inventory') then
        exports['qb-inventory']:AddItem(player.source, item, count, nil, data)
...
function InventorySetItem(player, item, data)
    ...
    elseif Config.inventory == 'qb-inventory' and IsResourceStarted('qb-inventory') then
        exports['qb-inventory']:SetItemData(player.source, item, 'info', data)
    end
end

function InventoryHasItem(player, item)
    if Config.inventory == "ox_inventory" then
        return (exports.ox_inventory:GetItem(player.source, item, nil, true) or 0) > 0
    elseif Config.inventory == 'codem-inventory' then
    ...
    elseif Config.inventory == 'qb-inventory' and IsResourceStarted('qb-inventory') then
        return exports['qb-inventory']:HasItem(player.source, item, 1)
    elseif Config.inventory == true then
