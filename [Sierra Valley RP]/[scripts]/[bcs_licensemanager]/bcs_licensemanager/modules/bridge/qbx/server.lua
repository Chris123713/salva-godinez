...
function GetPlayerFromId(source)
    ...
    if Player then
        ...
        Player.HasItem = function(item)
            return exports.ox_inventory:GetItem(Player.PlayerData.source, item, nil, true) > 0
        end
        return Player
    else
        return nil
...
function RemoveMoney(xPlayer, type, amount)
    ...
    print('Removing', amount, type, 'from', xPlayer.PlayerData.source)
...
function GetSex(xPlayer)
    return xPlayer.PlayerData.charinfo.gender == 1 and 'f' or 'm'
end
...
function RetrieveLicense(xPlayer, license)
    if GetAccount(xPlayer, 'cash') >= Config.Licenses[license].lostPrice then
        local licenseData = MySQL.scalar.await(
            ("SELECT `data` FROM %s WHERE `owner` = ? AND `license` = ?"):format(Config.Database.license), {
                xPlayer.PlayerData.citizenid,
                license
            })
            
        if licenseData then
            local data = json.decode(licenseData)
