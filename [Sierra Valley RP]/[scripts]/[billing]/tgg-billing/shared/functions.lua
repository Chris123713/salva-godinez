...
---@return boolean
function IsCompanyReceivable(jobIdentifier)
    local companyConfig = GetCompanyConfig(jobIdentifier)
    return companyConfig.canReceiveCompanyInvoices == true
end

---@param jobIdentifier string
---@return boolean
function IsCompanyInvoiceFeatureEnabled(jobIdentifier)
    -- Company invoice features are disabled for 'other' category
    if jobIdentifier == 'other' then
        return false
    end
    local companyConfig = GetCompanyConfig(jobIdentifier)
    return companyConfig.canReceiveCompanyInvoices == true
