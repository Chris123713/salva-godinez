...
lib.callback.register('billing:server:get-available-companies', function(source)
    local availableCompanies = {}
    local sourcePlayer = Framework?.GetPlayerFromId(source)
    if not sourcePlayer then return availableCompanies end

    local sourceJob = sourcePlayer.job?.name or sourcePlayer.getJob()?.name

    for _, company in pairs(Config.Companies) do
        -- Exclude 'other' category and only include companies with canReceiveCompanyInvoices enabled
        if company.jobIdentifier ~= 'other' and company.canReceiveCompanyInvoices == true and company.jobIdentifier ~= sourceJob then
            table.insert(availableCompanies, {
                id = company.jobIdentifier,
                name = company.name,
                type = 'company',
                companyId = company.jobIdentifier,
                citizenId = company.jobIdentifier -- Use jobIdentifier as citizenId for Select component
            })
        end
    end
