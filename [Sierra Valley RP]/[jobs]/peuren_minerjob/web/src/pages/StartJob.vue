<script setup>
    ...
    const CancelJob = async (groupId) => {
        const result = await fetchNui('cancel-job', { id: groupId, owner: isOwner.value, finished: false });
        if (!result) return;
    ...
    const LeaveJob = async (groupId) => {
        const result = await fetchNui('leave-job', { id: groupId, owner: isOwner.value, finished: true });
        if (!result) return;
        globalState.pageData.value.current.args.started = false;
        globalState.pageData.value.current = null;
        globalState.changeCurrentTab('job');
...
<template>
    ...
    <div v-else class="Container" :class="{ 'blurred': globalState.currentTab.value === 'select-jobs' }">
        <!-- Control Panel -->
                ...
                <div class="stats-grid">
                    <div class="stat-box">
                        ...
                        <div class="stat-content">
                            <span class="stat-value">{{ globalState.pageData.value.current.args.crystals + globalState.pageData.value.current.args.rocks }}</span>
