...
eventController<RequestUploadToken, string>(
  ...
  async ({ ctx, body, send }) => {
    function uploadCallback(
      ...
      if (!playerSource) {
        console.warn(`[screencapture] Attempted to send upload complete event to null/undefined player source (correlationId=${correlationId}, playerSource=${playerSource}). Player may have disconnected.`);
        return;
      }
