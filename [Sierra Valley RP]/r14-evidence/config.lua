...
-- Detect qbx_core first, then fall back to qb-core
...
Config.EvidenceLimits = {
    Max = 1000, -- max number of evidence pieces held on the server (reduced from 2000 for performance)
    RemoveAfterLimit = 100, -- number of evidence pieces removed when hitting the max limit
}

Config.EvidenceFadeTime = {
    Casings = 30, -- time, in minutes, that casings remain before being removed (reduced from 60 for performance)
    Impact = 30, -- time, in minutes, that bullet impacts remain in world before being removed (reduced from 45)
    Blood = 30, -- time, in minutes, that blood evidence remains before being removed (reduced from 60)
...
Config.Bleed = {
    ...
    DamageChance = 55, -- the chance a damage event causes a player to bleed (increased from 40 to reduce blood creation events)
}
...
Config.Camera = {
    ...
    [8] = {Path = 'SA_NSS/LSPD/', Password = 'lspd69', Auth = 'LEO'}, -- LSPD evidence upload channel with password
    [9] = {Path = 'SA_NSS/LSCSO/', Password = 'lscso69', Auth = 'LEO'}, -- LSCSO evidence upload channel with password
}
...
-- Provide a safe default for missing caliber lookups so the script doesn't spam missing-caliber warnings
setmetatable(Config.Calibers, {
    __index = function()
        return 'Unknown'
    end
})
