...
Citizen.CreateThreadNow(function()
    ...
    -- Add garage column if it doesn't exist (compatible with older MySQL versions)
    local garageExists = MySQL.single.await([[
    ...
    if not garageExists or garageExists.count == 0 then
        MySQL.query.await([[
            ALTER TABLE `properties`
            ADD COLUMN `garage` JSON AFTER `stash_options`
        ]])
    end
