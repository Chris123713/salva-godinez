...
function functions.GetQBPlayers()
    ...
    -- Add array-like properties for JavaScript compatibility
    deprecatedPlayers.length = 0
    local index = 1
    for _ in pairs(deprecatedPlayers) do
        deprecatedPlayers.length = deprecatedPlayers.length + 1
    end
    
    return deprecatedPlayers
end

-- Only export to qb-core bridge, not qbx_core (since it already exists there)
AddEventHandler('__cfx_export_qb-core_GetQBPlayers', function(setCB)
    setCB(functions.GetQBPlayers)
end)
...
-- Only export to qb-core bridge, not qbx_core (since it already exists there)
AddEventHandler('__cfx_export_qb-core_CreateUseableItem', function(setCB)
    setCB(functions.CreateUseableItem)
end)
...
---@return PlayerData[]
function functions.GetPlayersData()
    return exports.qbx_core:GetPlayersData()
end

-- Only export to qb-core bridge, not qbx_core (since it already exists there)
AddEventHandler('__cfx_export_qb-core_GetPlayersData', function(setCB)
    setCB(functions.GetPlayersData)
end)
