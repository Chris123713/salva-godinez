...
-- Reload skin command handler - supports rcore_clothing
RegisterNetEvent('qbx_core:client:reloadSkin', function()
    if GetResourceState("rcore_clothing") ~= "started" then
        return
    end
    
    -- Try common rcore_clothing reload methods
    local success = false
    
    -- Try export methods
    if exports.rcore_clothing then
        if exports.rcore_clothing.reloadSkin then
            exports.rcore_clothing.reloadSkin()
            success = true
        elseif exports.rcore_clothing.loadSkin then
            exports.rcore_clothing.loadSkin()
            success = true
        elseif exports.rcore_clothing.reloadAppearance then
            exports.rcore_clothing.reloadAppearance()
            success = true
        end
    end
    
    -- Try event methods if exports didn't work
    if not success then
        TriggerEvent("rcore_clothing:reloadSkin")
        TriggerEvent("rcore_clothing:loadSkin")
        TriggerEvent("rcore_clothing:reloadAppearance")
        TriggerEvent("rcore_clothing:client:reloadSkin")
    end
end)