...
local function spawnLastLocation()
    ...
    -- Ensure character is visible after spawning
    Wait(500)
...
local function createCharacter(cid)
    ...
    if not newData then
        Notify(locale('error.character_creation_failed') or 'Character creation failed. Please try again.', 'error', 10000)
        DoScreenFadeIn(500)
        previewPed()
        return false
    end
...
RegisterNetEvent('qbx_core:client:spawnNoApartments', function() -- This event is only for no starting apartments
    ...
    destroyPreviewCam()
    SetEntityVisible(cache.ped, true, false)
    -- Ensure big map is disabled when spawning
    ...
    -- Ensure big map stays disabled after loading
    Wait(500)
    SetBigmapActive(false, false)
    TriggerEvent('qb-clothes:client:CreateFirstCharacter')
end)
...
RegisterNetEvent('qbx_core:client:playerLoggedOut', function()
    print('^3[QBX_CORE] playerLoggedOut event received^0')
    local invokingResource = GetInvokingResource()
    print(('^3[QBX_CORE] Invoking resource: %s^0'):format(tostring(invokingResource)))
    
    if invokingResource then 
        print('^1[QBX_CORE] Event blocked - not from server^0')
        return 
    end
    
    print('^2[QBX_CORE] Starting character selection process^0')
    
    -- End any existing tutorial session first
    NetworkEndTutorialSession()
    print('^3[QBX_CORE] Tutorial session ended^0')
    
    -- Use pcall to catch any errors
    local success, err = pcall(chooseCharacter)
    if not success then
        print(('^1[QBX_CORE] Error in chooseCharacter: %s^0'):format(tostring(err)))
        -- Try again after a delay
        Wait(1000)
        pcall(chooseCharacter)
    else
        print('^2[QBX_CORE] Character selection opened successfully^0')
    end
end)
