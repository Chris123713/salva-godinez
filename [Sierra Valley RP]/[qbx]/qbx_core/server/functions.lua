...
function GetUserId(identifier)
    if not QBX or not QBX.Players then return 0 end
...
function GetPlayer(source)
    if not QBX or not QBX.Players then return nil end
...
function CreateUseableItem(item, data)
    if not QBX or not QBX.UsableItems then
        lib.print.error(('Cannot create useable item %s - QBX.UsableItems not initialized'):format(item))
        return
    end
...
function CanUseItem(item)
    if not QBX or not QBX.UsableItems then return nil end
...
function IsOptin(source)
    -- Always return true for admins - opt-in is no longer required
    if IsPlayerAceAllowed(source --[[@as string]], 'admin') then return true end
    return false
end
...
function HasPrimaryGroup(source, filter)
    if not QBX or not QBX.Players or not QBX.Players[source] then return false end
...
function GetGroups(source)
    if not QBX or not QBX.Players or not QBX.Players[source] then return {} end
...
local function getPlayersData()
    if not QBX or not QBX.Players then return {} end
