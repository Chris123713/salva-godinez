...
lib.callback.register('qbx_core:server:createCharacter', function(source, data)
    -- Validate input data
    if not data or not data.firstname or not data.lastname or not data.nationality or data.gender == nil or not data.birthdate or not data.cid then
        lib.print.error(('Character creation failed for %s: Missing required fields'):format(GetPlayerName(source)))
        return nil
    end
    ...
    if not success then
        lib.print.error(('Character creation failed for %s: Login failed'):format(GetPlayerName(source)))
        return nil
    end

    -- Get the created player to return proper data
    local player = GetPlayer(source)
    if not player then
        lib.print.error(('Character creation failed for %s: Player not found after login'):format(GetPlayerName(source)))
        return nil
    end

    giveStarterItems(source)

    lib.print.info(('%s has created a character (Citizen ID: %s)'):format(GetPlayerName(source), player.PlayerData.citizenid))
    
    -- Return the player data structure
    return {
