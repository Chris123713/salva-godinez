...
function Login(source, citizenid, newData)
    ...
    if not QBX or not QBX.Players then
        lib.print.error('QBX.Players not initialized yet. Login called too early.')
        return false
    end
...
local function toPlayerJob(jobName, job, grade)
    -- Ensure job.grades exists
    if not job.grades then
        job.grades = {}
    end
    
    -- Get grade data, fallback to first available grade or default if grade doesn't exist
    local gradeData = job.grades[grade]
    if not gradeData then
        -- Try to find first available grade
        for gradeLevel, gradeInfo in pairs(job.grades) do
            gradeData = gradeInfo
            grade = gradeLevel
            break
        end
        -- If still no grade, create a default one
        if not gradeData then
            gradeData = {
                payment = 0,
                isboss = false,
    ...
    return {
        ...
        type = job.type,
        grade = {
            name = gradeData.name or 'No Grade',
            level = grade
        }
    }
end
...
function CheckPlayerData(source, playerData)
    ...
    assert(job.grades ~= nil, 'Job grades not found for job: ' .. (job.name or 'unknown'))
    local jobGrade = GetJob(playerData.job?.name) and playerData.job.grade.level or 0
    if not job.grades[jobGrade] then
        jobGrade = 0
    end
    -- Ensure we have a valid grade, fallback to first available grade if grade 0 doesn't exist
    local gradeData = job.grades[jobGrade]
    if not gradeData then
        -- Find first available grade
        for gradeLevel, grade in pairs(job.grades) do
            gradeData = grade
            jobGrade = gradeLevel
            break
        end
    ...
    assert(gang.grades ~= nil, 'Gang grades not found for gang: ' .. (gang.name or 'unknown'))
    local gangGrade = GetGang(playerData.gang?.name) and playerData.gang?.grade?.level or 0
    if not gang.grades[gangGrade] then
        gangGrade = 0
    end
    -- Ensure we have a valid grade, fallback to first available grade if grade 0 doesn't exist
    local gangGradeData = gang.grades[gangGrade]
    if not gangGradeData then
        -- Find first available grade
        for gradeLevel, grade in pairs(gang.grades) do
            gangGradeData = grade
            gangGrade = gradeLevel
            break
        end
        -- If still no grade, create a default one
        if not gangGradeData then
    ...
    playerData.gang = {
        ...
        label = gang.label,
        isboss = gangGradeData.isboss or false,
        bankAuth = gangGradeData.bankAuth or false,
        grade = {
            name = gangGradeData.name or 'No Grade',
            level = gangGrade
        }
    }
...
function Logout(source)
    ...
    print(('^3[QBX_CORE] Triggering playerLoggedOut for source: %s^0'):format(source))
    
    -- Trigger the logout event BEFORE removing the player
    -- This ensures the callback in chooseCharacter() can still access player data if needed
    TriggerClientEvent('qbx_core:client:playerLoggedOut', source)
    TriggerEvent('qbx_core:server:playerLoggedOut', source)

    -- Wait longer to ensure the event is processed before removing player
    Wait(500)
    print(('^3[QBX_CORE] Removing player %s from QBX.Players^0'):format(source))
...
function Save(source)
    if not QBX or not QBX.Players or not QBX.Players[source] then
        lib.print.error(('Cannot save player %s - player data not found'):format(source))
        return
    end
...
function DeleteCharacter(source, citizenid)
    ...
    local storedLicense = playerEntity?.license
    local success = false

    -- If player entity doesn't exist, try to delete anyway (cleanup orphaned data)
    if not playerEntity then
        ...
        if success then
            logger.log({
                ...
                color = 'red',
                message = ('**%s** deleted **%s** (character not found, cleanup)...'):format(GetPlayerName(source), citizenid, source),
            })
        end
        return success
    end

    -- Verify license matches before deletion - check both license and license2 against stored license
    -- Also verify by checking if character belongs to user's character list
    local licenseMatches = (license and license == storedLicense) or (license2 and license2 == storedLicense)
    
    -- Additional verification: check if character is in user's character list
    local userCharacters = storage.fetchAllPlayerEntities(license2 or '', license or '')
    local characterBelongsToUser = false
    for i = 1, #userCharacters do
        if userCharacters[i].citizenid == citizenid then
            characterBelongsToUser = true
            break
        end
    end

    if licenseMatches or characterBelongsToUser then
        success = storage.deletePlayer(citizenid)
        if success then
            logger.log({
                source = 'qbx_core',
...
function ForceDeleteCharacter(citizenid)
    local playerEntity = storage.fetchPlayerEntity(citizenid)
    local result = playerEntity?.license
    
    -- Check if player is online and drop them
