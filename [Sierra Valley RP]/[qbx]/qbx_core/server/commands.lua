...
-- Reload skin command - supports both illenium-appearance and rcore_clothing
lib.addCommand('reloadskin', {
    help = 'Reload your character appearance/skin'
}, function(source)
    -- Support for illenium-appearance
    if GetResourceState("illenium-appearance") == "started" then
        TriggerClientEvent("illenium-appearance:client:reloadSkin", source)
    end
    
    -- Support for rcore_clothing
    if GetResourceState("rcore_clothing") == "started" then
        TriggerClientEvent("qbx_core:client:reloadSkin", source)
    end
end)

-- Relog command - allows players to disconnect and reconnect
lib.addCommand('relog', {
    help = 'Return to character selection'
}, function(source)
    local player = GetPlayer(source)
    if not player then 
        Notify(source, 'Unable to relog at this time', 'error')
        return 
    end
    
    print(('^3[QBX_CORE] /relog command executed by source %s^0'):format(source))
    
    -- Use the same Logout function as /logout to return to character selection
    Notify(source, 'Returning to character selection...', 'success')
    
    -- Save player data before logout
    player.PlayerData.metadata.hunger = Player(source)?.state?.hunger or player.PlayerData.metadata.hunger
    player.PlayerData.metadata.thirst = Player(source)?.state?.thirst or player.PlayerData.metadata.thirst
    player.PlayerData.metadata.stress = Player(source)?.state?.stress or player.PlayerData.metadata.stress
    ...
    -- Remove player from QBX.Players
    QBX.Players[source] = nil
    GlobalState.PlayerCount -= 1
    
    -- Trigger multicharacter system
    TriggerClientEvent('0r-multicharacterv3:client:loadUserData', source)
    
    print(('^2[QBX_CORE] Player %s returned to character selection^0'):format(source))
end)