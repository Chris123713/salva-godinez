...
------------------------------------------------
-- EXPORT: sirenCheck - For Tommy's Radio integration
-- Returns true if any siren audio is currently playing
------------------------------------------------
exports('sirenCheck', function()
    ...
    -- Check main siren state (wail, yelp, priority, etc.)
    ...
    -- Check powercall/auxiliary siren state
    ...
    -- Check airhorn/manual siren state
    if state_airmanu[vehicle] then
        if state_airmanu[vehicle] == true or state_airmanu[vehicle] > 0 then
            siren = true
        end
    end
