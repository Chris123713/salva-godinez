...
function Handler:fixVehicle(vehicle, coords, fixtype)
    ...
    if fixtype == 'smallkit' or fixtype == 'bigkit' then
        if #(coords - offset) < 2.0 then
            ...
            if success then
                lib.callback('vehiclehandler:sync', false, function()
                    -- fully repair vehicle visuals and health for repair kits
                    ...
                    -- try to clear deformation: native + external resource (Kiminaze's VehicleDeformation)
