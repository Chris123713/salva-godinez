...
local QBCore = exports['qbx_core']:GetCoreObject() -- initialize qbx_core for Qbox framework
...
local function filterTimeFromDate (date)
    if not date then return "00:00:00" end -- handle nil date
    local time = date:sub(12, 19)
    return time
end
...
function monitor:log (source, job)
    ...
    -- if no clockin time stored, skip logging (player wasn't tracked going on duty)
    if not clockin_time then
        print('^3[browns_dutytime]^7 Skipping log for source '..tostring(source)..' - no clockin time recorded')
        return
    end
