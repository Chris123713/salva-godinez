...
local function SpawnVendors()
    ...
    local ok = pcall(function() exports['qb-target']:SpawnPed(vendors) end)
    if not ok then
        local waited = 0
        while waited < 5000 do
            if GetResourceState('qb-target') == 'started' then
                local ok2 = pcall(function() exports['qb-target']:SpawnPed(vendors) end)
        ...
        if waited >= 5000 then
            print("^1[r14-objects] qb-target SpawnPed export unavailable after waiting^7")
        end
    end
end
...
local function DeleteVendors()
    if (GetCurrentResourceName() == resourceName) then
        ...
        local ok, CurrentPeds = pcall(function() return exports['qb-target']:GetPeds() end)
        if ok and CurrentPeds then
            ...
            pcall(function() exports['qb-target']:RemoveSpawnedPed(DeleteList) end)
        else
            print("^1[r14-objects] qb-target GetPeds export unavailable when deleting vendors^7")
        end
    end
end
